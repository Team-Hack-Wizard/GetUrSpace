...
export default function AdminFacilities() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(userRef, async (userDoc) => {
      ...
      let curListData = await Promise.all(groups.map(async (group) => {
        return (
          {
            id: group.groupId,
            name: group.groupName,
            data: await Promise.all(group.facilities.map(async (facility) => {
              const facilityDoc = (await getDoc(doc(db, "facilities", facility.facilityId)));
              return ({
