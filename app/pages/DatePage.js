...
import { query, collection, where, getDocs, getDoc, doc, onSnapshot } from 'firebase/firestore';
...
export default function DatePage({ navigation, route }) {
  ...
  const handleContinue = async () => {
    ...
    const q = query(collection(db, 'bookings'), where('userId', '==', booking.userId),
      where('facilityId', '==', booking.facilityId), where('date', '>=', weekStart.format("YYYY-MM-DD")),
    ...
    if (snapshots.size >= maxHoursPerWeek) {
      ...
      Msg('Max Hours Per Week Reached',
